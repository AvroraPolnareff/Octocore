...
impl Push2 {
    ...
    pub fn connect(&mut self) {
        ...
        let device = context
            ...
            .find(|device| {
                ...
                descriptor.vendor_id() == ABLETON_VENDOR_ID
                    && descriptor.product_id() == PUSH_2_PRODUCT_ID
            })
            .expect("Ableton push 2 is not connected");
        let mut device_handle = device.open().unwrap(); //.expect("Error while opening a device connection");
    ...
    pub fn draw_image(&self, image: &mut [u8]) -> Result<usize, UsbError> {
        ...
        handle.write_bulk(1, &FRAME_HEADER, Duration::from_millis(16))?;
        handle.write_bulk(1, image, Duration::from_millis(16))
    }
}
