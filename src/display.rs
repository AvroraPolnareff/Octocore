...
pub fn render_param(name: &str, value: String, point: impl Into<Point>, canvas: &Canvas) {
...
pub fn render_image(
    ...
    match *page {
        Page::Op(x) => match *op_subpage {
            OpPage::Tone => {
                render_param(
                    "Volume",
                    fmt_float(params.ops[x as usize].volume.value()),
                    calc_param_pos(1.),
                    canvas,
                );
                render_param(
                    "Ratio",
                    fmt_float(params.ops[x as usize].ratio.value()),
                    calc_param_pos(2.),
                    canvas,
                );
            }
            OpPage::Amp => {
                render_param(
                    "Attack",
                    fmt_float(params.ops[x as usize].adsr_params.a.value()),
                    calc_param_pos(1.),
                    canvas,
                );
                render_param(
                    "Decay",
                    fmt_float(params.ops[x as usize].adsr_params.d.value()),
                    calc_param_pos(2.),
                    canvas,
                );
                render_param(
                    "Sustain",
                    fmt_float(params.ops[x as usize].adsr_params.s.value()),
                    calc_param_pos(3.),
                    canvas,
                );
                render_param(
                    "Release",
                    fmt_float(params.ops[x as usize].adsr_params.r.value()),
                    calc_param_pos(4.),
                    canvas,
                );
            }
